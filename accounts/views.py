...
from .serializers import (UserTypeSerializer, UserSerializer, PermissionSerializer,
                          UsersLocationSerializer, OtpMasterSerializer)
...
class UsersLocationViewSet(viewsets.ModelViewSet):
    queryset = UsersLocation.objects.all()
    serializer_class = UsersLocationSerializer
...
from rest_framework.authtoken.models import Token # We'll just return a mock token or manually create one, wait we didn't add rest_framework.authtoken to INSTALLED_APPS. We can just return a simple JSON response for now as the frontend just needs success.
...
def authenticate_user(request):
    ...
    try:
        ...
        # Auto-provision 'admin' user for testing if it doesn't exist
